...
const ShopOpenCloseButton = ({ onClick }) => {
  ...
  return (
    <ToggleContainer>
      ...
      <SwitchButton isOnline={isOnline} onClick={handleToggle}>
        <SwitchThumb isOnline={isOnline} />
      </SwitchButton>
    </ToggleContainer>
  );
};
...
const SwitchButton = styled.button`
  width: 60px;
  height: 32px;
  background-color: ${({ isOnline }) => (isOnline ? '#4CAF50' : '#ccc')};
  border-radius: 999px;
  border: none;
  position: relative;
  cursor: pointer;
...
const SwitchThumb = styled.div`
  position: absolute;
  top: 3px;
  left: ${({ isOnline }) => (isOnline ? '32px' : '3px')};
  width: 26px;
  height: 26px;
